			...
			<?php
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
						...
						$result = $db->query("SELECT * FROM request WHERE start_date = '".$currentDate."' AND verification = 1");
...
function getEvents($date = ''){
	...
	$result = $db->query("SELECT * FROM request WHERE start_date = '".$date."' AND verification = 1");
	if($result->num_rows > 0){
		$eventListHTML = '<h2>Records on '.date("l, d M Y",strtotime($date)).'</h2>';
		$eventListHTML .= '<ul>';
		while($row = $result->fetch_assoc()){
            $eventListHTML .= '<li>' .$row['name']. '</li>';
						$eventListHTML .= '<li>'.$row['reason'].'</li>';
        }
		$eventListHTML .= '</ul>';
	}
	echo $eventListHTML;
